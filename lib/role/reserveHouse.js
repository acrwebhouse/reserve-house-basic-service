...
function editReserveHouse(id,client,host,houseId,state,type,clientName,clientPhone, callback) {
    if (utilsValue.isValid(id) && id.length == 24){
        ...
        if(utilsValue.isValid(state) && utilsValue.isNumber(state*1)){
            updateData.state = state * 1
        }

        if(utilsValue.isValid(client)){
            updateData.client = ObjectId(client)
        }
        if(utilsValue.isValid(host)){
            updateData.host = ObjectId(host)
        }
        if(utilsValue.isValid(houseId)){
...
function getReserveHouses(host,client,states,type,skip,limit,sort,callback){
    ...
    if(utilsValue.isValid(client) && client.length == 24){
        match.client = ObjectId(client)
    }
